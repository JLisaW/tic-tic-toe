...
/***/ function(module, exports, __webpack_require__) {
	...
	var makeMove = function makeMove(array, tile, player) {
	  if (currentPlayer === 'x') {
	  ...
	  api.updateGame(moveData).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
	};

	var winningPlayer = void 0;
	var winningCombo = function winningCombo(array) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onIndex = function onIndex(event) {
	  event.preventDefault();
	  api.index().then(ui.onIndexSuccess).catch(ui.onError);
	};

	var onGetGame = function onGetGame(event) {
	  event.preventDefault();
	  var game = getFormFields(event.target).game;

	  if (game.id.length !== 0) {
	    api.show(game.id).then(ui.onSuccess).catch(ui.onError);
	  } else {
	...
	var createGame = function createGame(event) {
	  ...
	  api.createGame()
	  // game.reenableClick()
	  .then(ui.createGameSuccess).catch(ui.createGameFailure);
	};
